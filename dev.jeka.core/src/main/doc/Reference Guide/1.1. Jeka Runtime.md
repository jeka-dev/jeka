...
* If no such class found, Jeka selects the built-in `JkClass` class.
...
The Jeka displays the effective path at the very start of the process if launched with `-LRI` option :

For example, `jeka help -LRI` will output :
