...
Jeka commands can be launched both from command line and from your IDE as regular Java main method.
...
* __In other cases :__ argument is considered as a command (method) to be invoked on the _command class_ instance.
...
The commandSet instantiation process is defined in `ork.jeka.tool.JkCommandSet#of` factory method. It consists in :
1. Creating a new _command class_ instance (Invoking default constructor).
2. Injecting defined options in public instance fields of the command class and in the imported ones.
...
7. Invoking `JkCommandSet#postSetup` on _command class_. This method might be override.
8. The commandSet is ready.

#### Invoke Methods Specified in Jeka Command Line arguments 

Once commandSet is ready, Jeka invokes instance methods mentioned in command line as `jeka myFistMethod mySecondMethod ...`. 
...
* project/modules mentioned in `@JkDefImport` annotations of your _def command classes_.
