...
* __Reference to module__ (represented by `JkModuleDependency`) hosted in a binary repository (Ivy or Maven for instance) : Jeka can consume and resolve transitively any artifact located in a repository as you would do with Maven or Ivy.

For the last, Jeka is using _Ivy 2.4.0_ under the hood. 
...
To label dependencies according their usage, Jeka uses the notion of _scope_ (represented by `JkScope` class). This notion is similar to the Maven scope.
