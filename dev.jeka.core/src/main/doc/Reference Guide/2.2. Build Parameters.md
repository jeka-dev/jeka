...
Jeka runs are parameterizable. One can retrieve values defined at runtime by reading :
...
Declare a Server field in your command class :
...
- -RunClass (shorthand -RC) : Force to use the specified class as the _command class_ to instantiate. It can be the short name of the class (without package prefix).
