...
public final class JkKotlinCompiler {
    ...
    private Result run(JkKotlinJvmCompileSpec compileSpec) {
        ...
        if (command != null) {
            JkLog.info("Use kotlin compiler : " + command + " with options " + this.options);
            kotlincProcess = JkProcess.of(command)
                    .addParams(this.jvmOptions.stream()
                            .map(JkKotlinCompiler::toJavaOption)
                            .collect(Collectors.toList()));
        } else {
            JkLog.info("Use kotlin compiler version " + jarsVersionAndTarget.version + " with options " + this.options);
        ...
        kotlincProcess
                    .addParams(toWindowsArgs(compileSpec.getOptions()))
