...
public class SourceParser2Test {

    @Test
    public void withOneImport() {
        final JkDependencySet dependencies = parseResource("with-1-import.javasource").dependencies;
        Assert.assertEquals(1, JkUtilsIterable.listOf(dependencies).size());
    }

    @Test
    public void with3MultiAnnoImports() {
        final JkDependencySet dependencies = parseResource("with-3-multi-Imports.javasource").dependencies;
    ...
    public void withoutImport() {
        final JkDependencySet dependencies = parseResource("without-import.javasource").dependencies;
        Assert.assertEquals(0, dependencies.getEntries().size());
    }

    @Test
    public void with2ProjectImports() {
        final LinkedHashSet<Path> projects = parseResource(("with-2-project-imports.javasource"))
                .dependencyProjects;
    ...
    private static ParsedSourceInfo parseResource(String resourceName) {
        String pathString = SourceParser2Test.class.getResource(resourceName).getFile();
        Path file = Paths.get(pathString);
        return SourceParser2.parse(file, Paths.get(""));
    }
