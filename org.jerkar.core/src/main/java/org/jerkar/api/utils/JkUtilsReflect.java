...
public final class JkUtilsReflect {
	...
	try {
	    ...
	    throw JkUtilsThrowable.unchecked(e,
		    "Error while invoking " + method + " with params " + Arrays.toString(params));
	}
    }
	...
	for (int i = 0; i < params.length; i++) {
	    result[i] = params[i] == null ? null
		    : params[i].getClass().getName() + ":" + params[i].getClass().getClassLoader();
	}
	return result;
    }
	...
	try {
	    ...
	    throw new IllegalStateException("No method " + name + " with argTypes " + toString(argTypes)
	...
	if (result.isEmpty()) {
	    throw new IllegalArgumentException(
		    "No " + (staticMethod ? "static" : "instance") + " method found on class " + className
			    + " for method " + methodName + " and param types " + Arrays.toString(types));
	} else if (result.size() > 1) {
	    throw new IllegalArgumentException("Several " + (staticMethod ? "static" : "instance")
		    + " methods match on class " + className + " for method " + methodName + " and param types "
		    + Arrays.toString(types) + ". You should use method #invoke(Method, Object[] args) instead.");
	}
	final Method method = result.iterator().next();
	return invoke(target, method, args);
    }

    private static Set<Method> findMethodsCompatibleWith(boolean staticMethod, Set<Method> methods, String methodName,
	    Class<?>[] argTypes) {
