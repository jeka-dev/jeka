...
public class JkTestSuiteResult implements Serializable {
	...
	public JkTestSuiteResult(Properties properties, String suiteName, int totaltestCount, int ignoreCount, Iterable<? extends TestCaseResult> testCaseResult, long durationInMillis) {
	...
	public static JkTestSuiteResult empty(Properties properties, String name, long durationInMillis) {
		return new JkTestSuiteResult(properties, name, 0,0, Collections.EMPTY_LIST, durationInMillis);
	}
	...
	static JkTestSuiteResult fromJunit4Result(Properties properties, String suiteName, Object result, long durationInMillis) {
		final Integer runCount = JkUtilsReflect.invoke(result, "getRunCount");
		final Integer ignoreCount = JkUtilsReflect.invoke(result,
				"getIgnoreCount");
		final List<Object> junitFailures = JkUtilsReflect.invoke(result,
				"getFailures");
	...
	private static JkTestSuiteResult.TestCaseFailure fromJunit4Failure(Object junit4failure) {
		final Object junit4Description = JkUtilsReflect.invoke(junit4failure,
				"getDescription");
		final String testClassName = JkUtilsReflect.invoke(junit4Description,
				"getClassName");
		final String testMethodName = JkUtilsReflect.invoke(junit4Description,
				"getMethodName");
		final Throwable exception = JkUtilsReflect
				.invoke(junit4failure, "getException");
		final ExceptionDescription description = new ExceptionDescription(exception);
		return new JkTestSuiteResult.TestCaseFailure(testClassName, testMethodName, -1,
				description);
	}

	static JkTestSuiteResult.TestCaseFailure fromJunit3Failure(Object junit3failure) {
		final Object failedTest = JkUtilsReflect.invoke(junit3failure,
				"failedTest");
		final Throwable exception = JkUtilsReflect.invoke(junit3failure,
				"thrownException");
		final ExceptionDescription description = new ExceptionDescription(exception);
		...
		return new JkTestSuiteResult.TestCaseFailure(failedTest.getClass().getName(), methodName,-1,
				description);
	}
