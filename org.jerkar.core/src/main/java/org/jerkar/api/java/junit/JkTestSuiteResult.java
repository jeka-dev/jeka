...
public class JkTestSuiteResult implements Serializable {
    ...
    public JkTestSuiteResult(Properties properties, String suiteName, int totaltestCount, int ignoreCount,
	    Iterable<? extends TestCaseResult> testCaseResult, long durationInMillis) {
	...
	return "" + runCount + " test(s) run, " + failureCount() + " failure(s), " + ignoreCount + " ignored. In "
		+ durationInMilis + " milliseconds.";
    }
	...
	private List<String> stackTracesAsStrings(String prefix) {
	    ...
	    if (cause != null) {
		result.addAll(cause.stackTracesAsStrings(
			"Caused by : " + cause.getClassName() + ": " + messageOrEmpty(cause.getMessage())));
	    }
	    return result;
	}
    ...
    static JkTestSuiteResult fromJunit4Result(Properties properties, String suiteName, Object result,
	...
	return new JkTestSuiteResult.TestCaseFailure(failedTest.getClass().getName(), methodName, -1, description);
    }
