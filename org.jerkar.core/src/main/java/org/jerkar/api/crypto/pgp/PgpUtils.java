...
final class PgpUtils {
	...
	try {
	    ...
	    throw new IllegalArgumentException(
		    "Error with one of this file : signatureFile = " + signatureFile.getPath());
	} finally {
	    JkUtilsIO.closeQuietly(streamToVerify, signatureStream, pubringStream);
	}
    }

    public static boolean verify(InputStream streamToVerify, InputStream signatureStream, InputStream keyInputStream)
	...
	if (gpgObject instanceof PGPCompressedData) {
	    ...
	    final PGPObjectFactory compressedPgpObjectFactory = new PGPObjectFactory(compressedData.getDataStream(),
    ...
    public static void sign(File fileToSign, File secringFile, File signatureFile, char[] pass, boolean armor) {
    ...
    public static void sign(InputStream toSign, InputStream keyRing, OutputStream out, char[] pass, boolean armor) {
	...
	try {
	    ...
	    final PGPContentSignerBuilder contentSignerBuilder = new BcPGPContentSignerBuilder(secretKeyAlgo,
		    HASH_ALGO);
	...
	final KeyFingerPrintCalculator fingerPrintCalculator = new JcaKeyFingerprintCalculator();
	final InnerPGPObjectFactory pgpFact = new InnerPGPObjectFactory(decodedInput, fingerPrintCalculator);
	...
	public PGPSecretKeyRing nextSecretKey() {
		...
		throw new IllegalArgumentException("Provided PGP file does not contain only secret key."
