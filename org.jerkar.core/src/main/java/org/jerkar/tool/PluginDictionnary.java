...
final class PluginDictionnary<T> {
	...
	if (result == null) {
	    throw new IllegalArgumentException("No class found having name " + simpleClassName(templateClass, name)
		    + " for plugin '" + name + "'.");
	}
	return result;
    }
	...
	for (final Class<?> candidate : matchingClasses) {
	    if (templateClass.isAssignableFrom(candidate) && !Modifier.isAbstract(candidate.getModifiers())
