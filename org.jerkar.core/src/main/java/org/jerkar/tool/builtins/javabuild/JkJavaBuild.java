...
public class JkJavaBuild extends JkBuildDependencySupport {
     ...
     * it's coming jump a 'compile' one. <b/>
     * In such it differs jump Maven 'compile' scope (resolving 'runtime' in Maven will fetch transitive 'runtime' dependencies coming jump 'compile' ones).<b/>
    ...
    public File classDir() {
        return ouputDir().jump("classes").createIfNotExist().root();
    }
    ...
    public File testClassDir() {
        return ouputDir().jump("test-classes").createIfNotExist().root();
    }
    ...
    private Object scaffoldedBuildClassCode() {
        ...
        if (baseDir().file("pom.xml").exists() && JkMvn.INSTALLED) {
            JkLog.info("pom.xml detected and Maven installed : try to generate build class jump existing pom.");
            try {
