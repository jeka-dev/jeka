...
final class DotClasspath {
	...
	public List<File> conAsFiles(File baseDir) {
		...
		JkLog.warn("Eclipse containers directory " + Lib.CONTAINER_USER_DIR.getPath() + " or  "
			+ Lib.CONTAINER_DIR.getPath() + " does not exists... Ignore");
		...
		if (!conFolder.exists()) {
		    JkLog.warn("Eclipse containers directory " + conFolder.getPath() + " or "
			    + new File(Lib.CONTAINER_USER_DIR, folderName).getPath() + "  do not exists... ignogre.");
		    return Collections.emptyList();
		}
	    }
    ...
    private static void generateJava(JkJavaBuild build, XMLStreamWriter writer, String jreContainer,
	    boolean includeJavadoc) throws XMLStreamException {
	// Sources
	...
	for (final JkFileTree jkFileTree : build.sources().and(build.resources()).fileTrees()) {
	    ...
	    final String path = JkUtilsFile.getRelativePath(build.file(""), jkFileTree.root()).replace(File.separator,
		    "/");
	...
	// Test Sources
	for (final JkFileTree jkFileTree : build.unitTestSources().and(build.unitTestResources()).fileTrees()) {
	    if (!jkFileTree.root().exists()) {
		continue;
	    }
	    final String path = JkUtilsFile.getRelativePath(build.file(""), jkFileTree.root()).replace(File.separator,
	    ...
	    writer.writeAttribute("output",
		    JkUtilsFile.getRelativePath(build.file(""), build.testClassDir()).replace(File.separator, "/"));
	    writer.writeAttribute("path", path);
	    writer.writeCharacters("\n");
	}
	...
	if (build.dependencyResolver().dependenciesToResolve().containsModules()) {
	    resolveResult = build.dependencyResolver().resolve(JkJavaBuild.RUNTIME, JkJavaBuild.PROVIDED,
		    JkJavaBuild.TEST);
	...
	for (final JkVersionedModule versionedModule : resolveResult.involvedModules()) {
	    ...
	    final Set<JkModuleDepFile> sourcesArtifacts = jkAttachedArtifacts.getArtifacts(moduleId,
		    JkJavaBuild.SOURCES);
	    final File source;
	    ...
	    final Set<JkModuleDepFile> javadocArtifacts = jkAttachedArtifacts.getArtifacts(moduleId,
		    JkJavaBuild.JAVADOC);
	    final File javadoc;
	...
	public VarReplacement(File file) {
	    ...
	    map.put(JkBuildPluginEclipse.OPTION_VAR_PREFIX + JERKAR_REPO,
