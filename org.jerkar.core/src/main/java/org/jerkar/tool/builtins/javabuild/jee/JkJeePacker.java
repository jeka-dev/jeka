...
public class JkJeePacker {
	...
	if (!new File(webappSrc, "WEB-INF/web.xml").exists()) {
	    throw new IllegalStateException("The directory " + webappSrc.getPath() + " does not contains WEB-INF"
		    + File.separator + "web.xml file");
	}
	final JkPath path = build.depsFor(JkJavaBuild.RUNTIME);
	final JkFileTree dir = JkFileTree.of(warDirDest).importDirContent(webappSrc).from("WEB-INF/classes")
		.importDirContent(build.classDir()).from("../lib").importFiles(path);
