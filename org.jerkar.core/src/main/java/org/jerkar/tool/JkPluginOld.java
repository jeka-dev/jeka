 ...
 * <p>
 * All plugin classes must be named as <code>JkPluginXxxxxx</code> where xxxxxx stands for the plugin name. This
 * is necessary in order to find plugin classes from its name in the classpath.
 * <p>
 ...
 *
 * <h5>Configuration</h5>
 * One may need to preConfigure a plugin in the build class itself in order to avoiding to mention the configuration in the
 * command line each time it is invoked. For such build classses, writer has to instantiate and preConfigure programmatically
