...
final class CommandLine {
	...
	for (String word : words) {
		...
		if (equalIndex <= -1) {
		    ...
		    final String key = JkUtilsString
			    .substringBeforeFirst(JkUtilsString.substringAfterFirst(word, PLUGIN_SYMBOL), "=");
		    final String value = word.substring(equalIndex + 1);
	...
	private static boolean isPluginMethodInvokation(String word) {
	    ...
	    return JkUtilsString.countOccurence(word, PLUGIN_SYMBOL_CHAR) == 1 && !word.startsWith(PLUGIN_SYMBOL)
		    && !word.endsWith(PLUGIN_SYMBOL);
	}
