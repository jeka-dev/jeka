				...
				if (equalIndex <= -1) {  // no '=' so we just associate the key with a null value
					...
					if (!key.contains(PLUGIN_SYMBOL)) {
						...
						if (value.endsWith(ALL_BUILD_SYMBOL)) {
							result.put(key, JkUtilsString.substringBeforeLast(value, ALL_BUILD_SYMBOL));
						} else if (master){
		...
		for (String word : words) {
			...
			if (MethodInvocation.isPluginMethodInvokation(word)) {
				...
				if (equalIndex <= -1) {
					...
					final String key = JkUtilsString.substringBeforeFirst( JkUtilsString.substringAfterFirst(word, PLUGIN_SYMBOL), "=");
					final String value = word.substring(equalIndex+1);
	...
	public static final class MethodInvocation {
		...
		public static MethodInvocation parse(String word) {
			if (isPluginMethodInvokation(word)) {
				return pluginMethod(
						JkUtilsString.substringBeforeFirst(word, PLUGIN_SYMBOL),
		...
		public static MethodInvocation pluginMethod(String pluginName, String methodName) {
			JkUtilsAssert.isTrue(pluginName != null && !pluginName.isEmpty(), "PluginName can' t ne null or empty");
		...
		private static boolean isPluginMethodInvokation(String word) {
			...
			return JkUtilsString.countOccurence(word, PLUGIN_SYMBOL_CHAR) == 1 && !word.startsWith(PLUGIN_SYMBOL) && !word.endsWith(PLUGIN_SYMBOL);
		}
