...
In Jerkar context, a _dependency_ is something that can be resolved to a set of files by a `JkDependencyResolver`.
...
For the last, Jerkar is using _Ivy 2.4.0_ under the hood. 
...
To label dependencies according their usage, Jerkar uses the notion of _scope_ (represented by `JkScope` class). This notion is similar to the Maven scope.
...
To sign with PGP, no need to have PGP installed on Jerkar machine. Jerkar uses <a href="https://www.bouncycastle.org/">Bouncy Castle</a> internally to sign artifacts.
