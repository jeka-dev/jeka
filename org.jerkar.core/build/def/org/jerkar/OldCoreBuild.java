 ...
 * manager.
 */
public class OldCoreBuild extends OldAbstractBuild {
    ...
    OldCoreBuild() {
        distripZipFile = ouputFile("jerkar-distrib.zip");
        distribFolder = ouputFile("jerkar-distrib");
    ...
    public static void main(String[] args) {
        JkInit.instanceOf(OldCoreBuild.class, args).doDefault();
    }
    ...
    protected JkManifest jarManifest() {
        ...
        if (version().isSnapshot()) {
            version = version + " - built at " + buildTime().toString();
        }
    ...
    private void distrib() {
        ...
        distrib.go("libs-sources").importDirContent(file("build/libs-sources"))
        .importFiles(packer.jarSourceFile());
        distrib.go("libs-javadoc").importFiles(this.javadocMaker().zipFile());
