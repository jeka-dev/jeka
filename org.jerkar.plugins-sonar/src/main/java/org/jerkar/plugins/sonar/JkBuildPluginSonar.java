...
@JkDoc({ "Add SonarQube capability to a build.", "The ananlysis is performed when the 'verify' method is invoked.",
...
public class JkBuildPluginSonar extends JkJavaBuildPlugin {
	...
	return JkSonar.of(build.moduleId().fullName(), build.moduleId().name(), build.effectiveVersion())
		.withProperties(JkOptions.getAllStartingWith("sonar.")).withProjectBaseDir(baseDir)
		.withBinaries(build.classDir()).withLibraries(libs).withSources(build.editedSources().roots())
		...
		.withProperty(JkSonar.JACOCO_REPORTS_PATH,
			JkUtilsFile.getRelativePath(baseDir, new File(build.testReportDir(), "jacoco/jacoco.exec")));
    }
