...
public class AntStyleBuild extends JkBuild {
    ...
    public void run() {
	jar();
	JkJavaProcess.of().withWorkingDir(jarFile.getParentFile()).andClasspath(classpath).runJarSync(jarFile);
    }
    ...
    public void junit() {
	jar();
	JkUnit.ofFork(classpath.and(jarFile)).withClassesToTest(JkFileTree.of(classDir).include("**/*Test.class"))
		.withReportDir(reportDir).withReport(JunitReportDetail.FULL).run();
    }
	...
	JkPublishRepo repo = JkRepo.maven(publishRepo).withCredential("myRepoUserName", "myRepoPassword")
		.asPublishRepo().withUniqueSnapshot(false).withSigner(pgp).andSha1Md5Checksums();
	...
	JkMavenPublicationInfo info = JkMavenPublicationInfo
		.of("my project", "my description", "http://myproject.github").withScm("http://scm/url/connection")
		.andApache2License().andGitHubDeveloper("myName", "myName@provider.com");
