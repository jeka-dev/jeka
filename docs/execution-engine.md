...
!!! note
    _Jeka_ organization provides a [plugin](https://plugins.jetbrains.com/plugin/13489-jeka/) for IntelliJ in order to make the following
    tasks smoother or transparent. 

    Here, we'll focus only on how to do it using command line.

!!! warning
    You need a JDK 8 or higher installed on your machine. By default, _Jeka_ will use the _Java_ executable 
    found in your _PATH_ environment variable. See later sections for changing this default.

There are two ways of using _jeka_ : by installing _Jeka_ itself of by using a template project containing the _Jeka_ wrapper.
...
### Use a Template Wrapper Project

An alternative of installing _Jeka_ is to clone a project template embedding the wrapper. 

* Clone template blank project from _https://github.com/jerkar/blank-template-project.git_
