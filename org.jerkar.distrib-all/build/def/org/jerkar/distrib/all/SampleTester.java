...
class SampleTester {
	...
	SampleTester(JkFileTree buildDir) {
		...
		this.sampleDependeeBaseDir = buildDir
				.from("../org.jerkar.samples-dependee");
	...
	void doTest() {
		...
		testSamples("", "doPublish",
				"-repo.publish.url=" + file.getAbsolutePath());
		File file2 = new File("build/output/test-out/mavenrepo-release");
		testSamples("", "doPublish", "-version=1.0", "-repo.release.url="
				+ file2.getAbsolutePath());
		File file3 = new File("build/output/test-out/ivyrepo");
		testSamples("", "doPublish",
				"-repo.publish.url=ivy:" + file3.getAbsolutePath());
		testSamples("IvyPublishBuild", "doPublish", "-repo.publish.url=ivy:"
	...
	private void testSamples(String className, String... args) {
		...
		JkProcess
				.of(launchScript.getAbsolutePath())
				.withWorkingDir(sampleBaseDir.root())
				.withParametersIf(!JkUtilsString.isBlank(className),
						"-buildClass=" + className).andParameters(args)
	...
	private void testFork() {
		...
		JkUtilsAssert.isTrue(output.file("test-reports/junit").exists(),
